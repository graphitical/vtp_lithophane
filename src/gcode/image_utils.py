...
class LithophaneImage:
    ...
    def _load_and_process_image(self):
        ...
        try:
            ...
            # Ensure image is in a mode suitable for black/white check or grayscale
            # Convert to 'L' (grayscale) for consistent 0-255 values
    ...
    def get_pixel_value(self, query_x_mm: float, query_y_mm: float, binarize: bool = True) -> int | float:
        """
        Maps a physical (X, Y) coordinate within the print bounds to an image pixel.
        Returns the binary value (0 for black, 1 for white) if binarize is True.
        Returns the pixel value mapped to a 0-1 range (0 for black/0, 1 for white/255)
        if binarize is False.
        Since the image is scaled to fill the physical bounds, there is no 'white space'
        outside the image within these bounds.
        ...
        if self._image is None:
            ...
            return 1.0  # Return white/max value if not loaded

        # Check if query point is outside the defined physical print bounds
        if not (0 <= query_x_mm < self.physical_print_width_mm and
                0 <= query_y_mm < self.physical_print_height_mm):
            # Query is outside the total print volume
            return 1.0  # Treat as white/max value outside print volume

        img_width_px, img_height_px = self._image.size

        # --- Map query physical coordinate to pixel coordinate ---
        # No offset needed as image fills the physical bounds
        # Invert Y coordinate for Pillow (top-left origin) from physical (bottom-left origin)
        # (self.physical_print_height_mm - query_y_mm) gives distance from top of physical bounds
        pixel_x = min(int(query_x_mm * self.scale_x_mm_to_px),
                      img_width_px - 1)
        pixel_y = min(int((self.physical_print_height_mm - query_y_mm)
                      * self.scale_y_mm_to_px), img_height_px - 1)
        ...
        try:
            ...
            return 1.0  # Treat as white/max value on error

        # --- Return value based on binarize flag ---
        # Map 0-255 pixel value to 0-1 range
        pixel_value_normalized = pixel_value / 255.0

        if binarize:
            # Round to nearest integer (0 or 1) for binarization
            return round(pixel_value_normalized)
        else:
...
if __name__ == '__main__':
    ...
    try:
        ...
        ax_binary.imshow(lithophane_image_obj_binary._image, extent=[
                         0, physical_w, 0, physical_h_binary], origin='lower', cmap='gray')
        ...
        for i, (qx, qy) in enumerate(query_points_binary):
            ...
            actual_pixel_value = "N/A"
            actual_pixel_value_norm = "N/A"
            try:
                if 0 <= qx < physical_w and 0 <= qy < physical_h_binary:
                    ...
                    if px >= 0 and py >= 0:
                        actual_pixel_value = lithophane_image_obj_binary._image.getpixel(
                            (px, py))
                        actual_pixel_value_norm = actual_pixel_value / 255.0
                    else:
                        actual_pixel_value = "Negative Coords"
                        actual_pixel_value_norm = "Negative Coords"

                # If outside print bounds, leave as "N/A"
            except Exception as e:
                actual_pixel_value = f"Error: {e}"
                actual_pixel_value_norm = f"Error: {e}"

            # Safely format actual_pixel_value_norm
            norm_display = f"{actual_pixel_value_norm:.2f}" if isinstance(
                actual_pixel_value_norm, (int, float)) else str(actual_pixel_value_norm)

            annotation_text = f"Q{i+1}: ({qx:.1f}, {qy:.1f}) mm\nReturned: Bin={val_bin}, Raw={val_raw:.2f}\nActual Pixel (0-255): {actual_pixel_value}, Norm: {norm_display}"
        ...
        # Display the image in the background, scaled to physical dimensions
        # Need to flip the image vertically for correct orientation on plot
        ax_gray.imshow(lithophane_image_obj_gray._image, extent=[
                       0, physical_w, 0, physical_h_gray], origin='lower', cmap='gray')
        ...
        for i, (qx, qy) in enumerate(query_points_gray):
            ...
            try:
                if 0 <= qx < physical_w and 0 <= qy < physical_h_gray:
                    ...
                    # Ensure pixel coordinates are non-negative before getting pixel
                    if px >= 0 and py >= 0:
                        actual_pixel_value = lithophane_image_obj_gray._image.getpixel(
                            (px, py))
                        actual_pixel_value_norm = actual_pixel_value / 255.0
                    else:
                        actual_pixel_value = "Negative Coords"
